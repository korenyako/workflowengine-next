      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="w-full space-y-6">
            {/* Header */}
            <div className="flex items-center justify-between text-sm font-semibold text-slate-500 uppercase tracking-wider">
              <span>Library</span>
              ...
              return (
                <div key={bundle.name}>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className={`font-semibold ${bundle.highlighted ? 'text-slate-900' : 'text-slate-500'}`}>
                      {bundle.name}
                    </span>
                    <div className="flex gap-8">
                      <span className={`w-24 text-right font-mono text-sm ${bundle.highlighted ? 'font-semibold text-purple-400' : 'text-slate-500'}`}>
                  ...
                  <div className="relative h-10 bg-slate-200 rounded-lg overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center px-4">
                      <span className={`text-sm truncate ${barPercent > 30 ? 'text-slate-900' : 'text-slate-500'}`}>
                        {bundle.description}
                      </span>
                    </div>
                  </div>
          ...
          <div className="w-full">
            <div className="rounded-xl bg-slate-100/40 p-6 text-center">
              <h3 className="text-blue-600 text-base font-subtitle font-semibold tracking-wide mb-5">
                Why it matters
              </h3>
              <ul className="flex flex-wrap justify-center gap-x-8 gap-y-4">
                {whyItMatters.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    ...
                    <span className="text-slate-600 text-lg leading-snug">{item.text}</span>
                  </li>
                ))}
              </ul>
            </div>
