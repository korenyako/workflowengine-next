...
const HowToUseBlock: React.FC<HowToUseBlockProps> = ({
  ...
  const cardTitleColor = 'text-blue-300';
      ...
      <div className={`max-w-6xl mx-auto grid grid-cols-1 ${
        ...
        {items.map((item, idx) => (
          <div key={idx} className={`${cardBg} flex flex-col ${centered ? 'items-center text-center' : 'items-start text-left'} h-full`}>
            ...
            {item.text && <p className={`${centered ? '' : 'text-lg lg:text-xl'} ${isLightBg ? 'text-gray-600' : 'text-gray-300'} whitespace-pre-wrap break-words leading-normal`}>{item.text}</p>}
          </div>
        ))}
      </div>
    </section>
